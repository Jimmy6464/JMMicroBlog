    ...
    for (NSTextCheckingResult *cr in resultArray) {
        ...
        for (NSDictionary *items in nameArr) {
            if ([[items objectForKey:@"chs"] isEqualToString:nameStr]) {
                ...
                [image setAccessibilityIdentifier:nameStr] ;
...
{
    NSMutableAttributedString *resultAtt = [[NSMutableAttributedString alloc]initWithAttributedString:attributeStr];
    [attributeStr enumerateAttributesInRange:NSMakeRange(0, attributeStr.length) options:NSAttributedStringEnumerationReverse usingBlock:^(NSDictionary *attrs, NSRange range, BOOL *stop) {
        //key-NSAttachment
        ...
        if (textAtt)
        {
            ...
            NSString * text = [image accessibilityIdentifier];
